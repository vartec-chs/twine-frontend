...
import { getElement, useEvent } from '@siberiacancode/reactuse'
...
export const useInfiniteScroll = ((...params) => {
	...
	const onLoadMore = useEvent(async (event: Event) => {
		...
		setPreviousScrollerHeight(scrollHeight)
		...
		if (distances[direction] <= distance) {
			...
			if (direction === 'top') {
				console.log('srollHeight ', scrollHeight, 'previousScrollHeight ', previousScrollHeight)

				internalRef?.scrollTo({
					top: scrollHeight - previousScrollHeight - distances.top,
					left: 0,
					behavior: 'smooth',
				})
			}
	...
	useEffect(() => {
		...
		if (isFirstRender && direction === 'top') {
			internalRef?.scrollTo({
				top: 10,
			})
